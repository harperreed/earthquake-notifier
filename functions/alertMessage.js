 ...
 * "M5.8 · 23km NW Kofu · depth 12km · P1". This is sent first and never
 * depends on the AI summary, so a real quake always produces a notification.
 ...
 */
function formatAlertLine(magnitude, distanceKm, bearing, depthKm, priority) {
  const mag = `M${magnitude.toFixed(1)}`;
  const loc = `${Math.round(distanceKm)}km ${bearing} Kofu`;
  const depth = `depth ${Math.round(depthKm)}km`;
  return `${mag} · ${loc} · ${depth} · P${priority}`;
}

/**
 * Builds the terse alert line for one enriched USGS earthquake feature — the
 * shape index.js assembles, carrying the precomputed distance, depth, and
 * priority. The compass bearing is derived from the monitored point to the
 * quake's coordinates.
